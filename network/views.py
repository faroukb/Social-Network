...
class NewPostForm(forms.Form):
    ...
    post_text = forms.Field(widget=forms.Textarea(
        {'rows': '3', 'maxlength': 160, 'class': 'form-control', 'placeholder': "What's on your mind?"}), label="New Post", required=True)


class EditPostForm(forms.Form):
    
    id_post_edit_text = forms.Field(widget=forms.Textarea(
        {'rows': '3', 'maxlength': 160, 'class': 'form-control', 'placeholder': "What's on your mind?", 'id': 'id_post_edit_text'}), label="New Post", required=True)
...
def profile(request, username):
    ...
    return render(request, "network/profile.html", {
        "user_profile": profile_user, "posts": page_obj, "is_following": is_following, 'total_following': total_following, 'total_followers': total_followers, 'form': NewPostForm(), 'form_edit': EditPostForm()
    })
